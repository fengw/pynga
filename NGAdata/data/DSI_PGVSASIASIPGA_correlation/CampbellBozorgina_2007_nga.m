...
if (length(find(abs((period-T))<0.0001))==0)
    ...
    [SA_low,sigma_SA_low]=CampbellBozorgina_2007_nga(M,Rrup,siteprop,faultprop);
    siteprop.period=T_hi;
    [SA_high,sigma_SA_high]=CampbellBozorgina_2007_nga(M,Rrup,siteprop,faultprop);
    siteprop.period=T;
    ...
    elseif k1(i) <= V30 < 1100.
        fsite = (c10(i)+k2(i)*n)*log(V30/k1(i));   % bug !
    else    
    ...
    slnyB=sqrt(slny(i)^2-0.3^2);   % slnYB
    slnyA=sqrt(slny(find(period==0))^2-0.3^2);   % slnAb
    %sigma_sq=slny(i)^2+alpha1^2*slny(find(period==0))^2+2*alpha1*rhos(i)*slnyB*slnyA;  % bug !   
    sigma_sq=slny(i)^2+alpha1^2*slnyA^2+2*alpha1*rhos(i)*slnyB*slnyA;    
