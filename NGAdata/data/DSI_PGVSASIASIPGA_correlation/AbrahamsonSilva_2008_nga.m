function [SA,sigma_SA]=AbrahamsonSilva_2008_nga(M,Rrup,siteprop,faultprop,flag)
...
if T~=0 | Vs30~=1100;
    siteprop.period=0;
    siteprop.V30=1100;
    [PGA1100,sigma_PGA1100]=AbrahamsonSilva_2008_nga(M,Rrup,siteprop,faultprop,0);
    siteprop.period=T;
    siteprop.V30=Vs30;
end
...
if (length(find(abs((period-T))<0.0001))==0)
    ...
    %if Rx > W*cos(delta) | delta==90    % bug !!! delta should be delta*pi/180
    if Rx > W*cos(delta*pi/180.) | delta==90    
        T2 = 1;
    else
    ...
    if T <= TD | (Vs30==1100 & Z10==0)
        if flag == 1
        disp('T<=TD');
        disp( [Ztor PGA1100 f1 f5 f4 f6 f8 f10] );
        end
       ...
       %get the closest period to TD
        ...
        siteprop.V30 = 1100;
        siteprop.Z1pt0 = 0;
        [SaTdminus,sigma_SaTdminus]=AbrahamsonSilva_2008_nga(M,Rrup,siteprop,faultprop,0);
        
        Tdplus = period(length(find(period<TD))+1);
        siteprop.period=Tdplus;
        siteprop.V30 = 1100;
        siteprop.Z1pt0 = 0;
        [SaTdplus,sigma_SaTdplus]=AbrahamsonSilva_2008_nga(M,Rrup,siteprop,faultprop,0);
    ...
    sigma0 = s1*(M<5) + (s1+(s2-s1)/2*(M-5))*(5<=M<=7) + s2 *(M>7); 
    %sigma0_PGA = min(s1_PGA, max(s2_PGA, s1_PGA+(s2_PGA-s1_PGA)/2*(M-5) ) );
    sigma0_PGA = s1_PGA*(M<5) + (s1_PGA+(s2_PGA-s1_PGA)/2*(M-5))*(5<=M<=7) + s2_PGA *(M>7);
    if M<5 
        sigma0 = s1; 
        sigma0_PGA = s1_PGA;
    elseif M > 7
        ...
        simga0_PGA = s1_PGA + (s2_PGA-s2_PGA)/2 * (M-5);
    end
    ...
    tau0 = s3(i)*(M<5) + (s3(i)+(s4(i)-s3(i))/2*(M-5))*(5<=M<=7) + s4(i) *(M>7);
    %tau0_PGA = min(s3(2), max(s4(2), s3(2)+(s4(2)-s3(2))/2*(M-5) ) );
    tau0_PGA = s3(2)*(M<5) + (s3(2)+(s4(2)-s3(2))/2*(M-5))*(5<=M<=7) + s4(2) *(M>7);
    ...
    elseif M>7
        ...
        tau0 = s3(i)+(s4(i)-s3(i))/2*(M-5);
        tau0_PGA = s3(2)+(s4(2)-s3(2))/2*(M-5);
    end
